...
sig=$(cast sig "deriveBeaconId((uint256,address,bytes32,address,address))")
...
rm_beacon() {
  ...
  data=$(set -x; ethabi encode params \
    -v "(uint256,address,bytes32,address,address)" \
    "(${chainId:2},${airnode:2},${endpointId:2},${sponsor:2},${sponsorWallet:2})")
  beaconId=$(seth call $subapi $sig$data --chain $chain)
  (set -x; seth send $subapi "removeBeacon(uint256,bytes32)" $chainId $beaconId --chain $chain)
}
...
rm_beacon $chain $endpointId $fromChain
